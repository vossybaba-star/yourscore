...
export default function TeamScreen() {
  ...
  function quickMatch() {
    ...
    const cur = team;
    const matchId = `local-${cur.updatedAt}-${Math.floor(Math.random() * 1e6)}`;
    ...
    // A level 90' stands as a draw.
    const res = resolveMatch(cur.squad, opp.team.squad, matchId);
    ...
    saveTeam(res.outcome === "A" ? recordWin(cur) : res.outcome === "B" ? recordLoss(cur) : recordDraw(cur));
    // Watch the match play out, then the result screen.
