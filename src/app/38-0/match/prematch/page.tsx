  ...
  async function play() {
    ...
    try {
      ...
      saveLastMatch({
        ...
        outcome: m.outcome, goals: m.goals, report: m.report, playedAt: Date.now(),
      });
      ...
      <div className="min-h-[100dvh] pb-40" style={{ background: "#0a0a0f" }}>
        <div className="max-w-lg mx-auto px-4 pt-safe">
          <div className="pt-4 pb-2 text-center">
            ...
            <div className="font-body" style={{ fontSize: 13, color: "#8a948f" }}>Spin and draft a {LINE_LABEL[posCategory(slot.pos)]} ({slot.label})</div>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0" style={{ background: "linear-gradient(0deg,#0a0a0f 78%,transparent)", paddingBottom: "calc(env(safe-area-inset-bottom,0px) + 14px)" }}>
          <div className="max-w-lg mx-auto px-4 pt-3">
            {(spinning || reel) && (
              <div className="mb-3 rounded-2xl px-4 py-3 text-center" style={{ background: "#0e1611", border: `1px solid ${spinning ? "rgba(255,184,0,0.4)" : "rgba(255,184,0,0.3)"}` }}>
                <div className="font-display tracking-wide" style={{ fontSize: 22, color: spinning ? "#ffb800" : "#fff" }}>{reel?.club ?? "—"} <span style={{ color: "#8a948f", fontSize: 16 }}>{reel?.season ?? ""}</span></div>
              </div>
            )}
            {current && !spinning && (
              <div className="mb-3 flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
                {draftable.map((p) => (
                  <button key={p.id} onClick={() => draft(p)} className="flex-shrink-0 rounded-xl p-3 text-left active:scale-95 transition-transform" style={{ width: 132, background: "#15211a", border: "1px solid rgba(255,184,0,0.3)" }}>
                    ...
                    <div className="font-body" style={{ fontSize: 11, color: "#8a948f" }}>{p.season}</div>
                  </button>
                ))}
              </div>
            )}
    ...
    <div className="min-h-[100dvh] pb-28" style={{ background: "#0a0a0f" }}>
      <div className="max-w-lg mx-auto px-5 pt-safe">
        <div className="pt-6 text-center">
          <div className="font-body" style={{ fontSize: 12, color: "#8a948f", letterSpacing: 1 }}>MATCH PREVIEW</div>
          <div className="font-display tracking-wide leading-none mt-1" style={{ fontSize: 30, color: "#fff" }}>
            YOU <span style={{ color: "#8a948f", fontSize: 20 }}>vs</span> {opp.name}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <div>
            <div className="font-display tracking-wide mb-1" style={{ fontSize: 15, color: "#aeea00" }}>YOUR XI</div>
        ...
        <div className="mt-5 rounded-2xl p-4 text-center" style={{ background: "#080d0a", border: "1px solid rgba(255,184,0,0.25)" }}>
