        ...
        needsAuth ? (
          ...
          <div className="w-full max-w-sm">
            <div className="text-center mb-6">
              ...
              <p className="font-body mt-2" style={{ fontSize: 14, color: "#8a948f" }}>
                Sign in to claim your spot in this lobby.
              </p>
            </div>
            <AuthProviders nextPath={`/38-0/live/${code}`} />
            <Link href="/38-0" className="block text-center mt-5 font-body text-sm" style={{ color: "#8a948f" }}>
              ← Back to Draft XI
            </Link>
          </div>
        ) : (
          <div className="text-center">
            ...
            {/team/i.test(error) && <p className="mt-2 text-sm" style={{ color: "#9aa39d" }}><Link href="/38-0/play" className="underline">Build your XI</Link> first, then try again.</p>}
            <Link href="/38-0/live" className="mt-5 inline-block underline text-sm" style={{ color: "#8a948f" }}>← Back to Live</Link>
          </div>
        )
      ) : (
        <div className="text-center">
          <div className="mx-auto h-10 w-10 rounded-full animate-spin" style={{ border: "3px solid rgba(174,234,0,0.25)", borderTopColor: "#aeea00" }} />
          <p className="mt-5 font-body" style={{ color: "#9aa39d" }}>
            {phase === "picking"
              ? "Picking your team…"
              : phase === "retrying"
              ? "Joining lobby…"
