...
/**
 * /draft/leagues — your private leagues: create one (get a join code), join by
...
export default function Leagues() {
  ...
  async function create() {
    ...
    try {
      ...
      router.push(`/draft/league/${d.code}`);
    ...
    <div className="min-h-[100dvh] pb-28" style={{ background: "#0a0a0f" }}>
      <div className="max-w-lg mx-auto px-5 pt-safe">
        <div className="flex items-center justify-between pt-4 pb-2">
          <Link href="/draft/leaderboard" className="font-body text-sm" style={{ color: "#8888aa" }}>← Leaderboard</Link>
        </div>
          ...
          <>
            ...
            {leagues.length > 0 && (
              <div className="space-y-2">
                {leagues.map((l) => (
                  <Link key={l.id} href={`/draft/league/${l.code}`} className="flex items-center justify-between rounded-xl px-4 py-3 active:scale-[0.98] transition-transform"
