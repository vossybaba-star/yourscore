...
import { deleteLeague, leagueDetail, leaveLeague, renameLeague, setLeagueProfile, setVisibility } from "@/lib/fantasy/leagues";
...
export async function PATCH(req: NextRequest, { params }: { params: { code: string } }) {
  ...
  let body: { name?: unknown; isPublic?: unknown; bio?: unknown; links?: unknown };
  ...
  if (!hasName && !hasVisibility && !hasBio && !hasLinks) return NextResponse.json({ error: "Nothing to update" }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ ok: true, name, isPublic, bio, links });
  } catch (e) {
    if (e instanceof HttpError) return NextResponse.json({ error: e.message, code: e.code }, { status: e.status });
