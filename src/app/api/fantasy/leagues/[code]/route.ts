...
import { deleteLeague, leagueDetail, leaveLeague, renameLeague, setLeagueProfile, setVenueMeta, setVisibility } from "@/lib/fantasy/leagues";
...
export async function PATCH(req: NextRequest, { params }: { params: { code: string } }) {
  ...
  let body: { name?: unknown; isPublic?: unknown; bio?: unknown; links?: unknown; venueMeta?: unknown };
  ...
  const hasVenue = "venueMeta" in body;
  if (!hasName && !hasVisibility && !hasBio && !hasLinks && !hasVenue) return NextResponse.json({ error: "Nothing to update" }, { status: 400 });
  ...
  try {
    ...
    if (hasBio || hasLinks) {
      const r = await setLeagueProfile(user.id, params.code ?? "", { bio: body.bio, links: body.links });
      bio = r.bio; links = r.links;
    }
    if (hasVenue) {
      venueMeta = await setVenueMeta(user.id, params.code ?? "", body.venueMeta);
    }
    return NextResponse.json({ ok: true, name, isPublic, bio, links, venueMeta });
  } catch (e) {
    if (e instanceof HttpError) return NextResponse.json({ error: e.message, code: e.code }, { status: e.status });
