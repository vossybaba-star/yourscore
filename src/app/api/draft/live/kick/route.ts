...
import type { PenZone } from "@/lib/draft/pens";
...
export async function POST(req: NextRequest) {
  ...
  let body: { matchId?: string; round?: number; shot?: number } = {};
  ...
  if (!body.matchId || round < 1 || shot < 0 || shot > 5) {
    return NextResponse.json({ error: "Bad request" }, { status: 400 });
  }

  try {
    const match = await liveKick(createDraftDb(), body.matchId, user.id, round, shot as PenZone);
