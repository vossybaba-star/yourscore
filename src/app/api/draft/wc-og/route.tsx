...
export async function GET(req: NextRequest) {
  ...
  const resColor = (r: string) => (r === "L" ? "#ff4757" : r === "Q" ? "#ffd700" : "#00ff87");
      ...
      <div style={{ width: "1200px", height: "630px", display: "flex", flexDirection: "column", background: "linear-gradient(135deg, #0a0a0f 0%, #12121e 100%)", padding: "56px 64px", fontFamily: "sans-serif", position: "relative" }}>
        ...
        <div style={{ display: "flex", marginTop: 36, flex: 1 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", width: 560, justifyContent: "center" }}>
            ...
            <div style={{ fontSize: 30, color: "#cfcfe6", marginTop: 18, display: "flex" }}>{sub}</div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", flex: 1, marginLeft: 40, justifyContent: "center" }}>
            {rows.map((row, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "12px 0" }}>
                <div style={{ display: "flex", fontSize: 24, color: "#8888aa", width: 150 }}>{row.label}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 30, color: "#cfcfe6", display: "flex" }}>Pick a nation. Win the World Cup.</div>
          <div style={{ fontSize: 30, color: "#00ff87", fontWeight: 700 }}>yourscore.app/38-0/wc</div>
        </div>
