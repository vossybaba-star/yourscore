...
export const FAME_WEIGHTS = { ownership: 0.4, price: 0.4, minutes: 0.2 };

export interface FameIndex {
  /** 0–100 fame for a player id (0 if unknown). */
  fame(id: number): number;
}
...
export function buildFameIndex(players: readonly Player[]): FameIndex {
  if (players.length === 0) return { fame: () => 0 };
  ...
  for (const p of players) {
    ...
    const f =
      FAME_WEIGHTS.ownership * o + FAME_WEIGHTS.price * pr + FAME_WEIGHTS.minutes * m;
    map.set(p.id, Math.round(clamp01(f) * 100));
  }
  return { fame: (id: number) => map.get(id) ?? 0 };
}
 ...
 */
export function comparisonDifficulty(
  fameA: number,
  fameB: number,
  valueCloseness: number,
): number {
