...
import type { SeasonStanding, SeasonTopScorer, SmSeason } from "./history";
import { shortSeasonName } from "./history";
...
export interface TriviaOpts {
  seed: string;
  /** Difficulty midpoint: how many years back reads as "hard" (default 15). */
  hardYearsAgo?: number;
  /** Current year for era difficulty (pass in — keeps the module Date-free). */
  nowYear: number;
}

/** Era difficulty: this season = ~20, `hardYearsAgo`+ = ~90. */
export function eraDifficulty(startYear: number, nowYear: number, hardYearsAgo = 15): number {
...
export function championQuestion(h: SeasonHistory, opts: TriviaOpts): GateQuestion | null {
  ...
  if (h.standings.filter((s) => s.position === 1).length !== 1) return null;
  const rand = seededRng(`${opts.seed}:champion:${h.season.id}`);
  ...
  return {
    ...
    prompt: `Who won the Premier League in ${shortSeasonName(h.season.name)}?`,
    options,
...
export function topScorerQuestion(h: SeasonHistory, opts: TriviaOpts): GateQuestion | null {
  ...
  if (second.goals === first.goals) return null;
  const rand = seededRng(`${opts.seed}:topscorer:${h.season.id}`);
  const options = shuffle(
    [first, second, third, fourth].map((s) => ({ id: s.playerId, label: s.name })),
    rand,
  );
  return {
    format: "classic-trivia",
    prompt: `Who was the Premier League's top scorer in ${shortSeasonName(h.season.name)}?`,
    options,
