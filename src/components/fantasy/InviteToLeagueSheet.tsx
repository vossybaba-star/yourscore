...
export function InviteToLeagueSheet({ inviteeId, inviteeName, onClose }: {
  ...
  // Portal to <body>: the sheet is position:fixed, and the feed's pull-to-refresh
  // wrapper has a transform, which would otherwise become the containing block and
  // trap the overlay inside the feed instead of the viewport.
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
          <div className="font-display" style={{ fontSize: 17, color: INK }}>Invite {inviteeName}</div>
