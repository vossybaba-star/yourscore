...
// Facebook's OAuth dialog doesn't understand Google's `prompt=select_account`
// (it has its own re-auth params) — send it only to the providers it's for.
const queryParamsFor = (provider: Provider) =>
  provider === "facebook" ? undefined : { prompt: "select_account" };
  ...
  async function signIn() {
    ...
    if (isNative()) {
      ...
      const { data, error } = await sb.auth.signInWithOAuth({
        provider,
        options: { redirectTo, skipBrowserRedirect: true, queryParams: queryParamsFor(provider) },
      });
      if (error || !data?.url) {
        setLoading(false);
    ...
    await sb.auth.signInWithOAuth({
      provider,
      options: { redirectTo, queryParams: queryParamsFor(provider) },
    });
  }
...
export function AuthProviders({ nextPath }: { nextPath?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Facebook stays hidden until the Supabase provider is enabled — flipping
          NEXT_PUBLIC_FACEBOOK_LOGIN=1 without that config live would render a
