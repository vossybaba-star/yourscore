/**
 * send-wc-quiz-day2.mjs
 *
 * Campaign: WC Quiz Series — Day 2 (template 17)
 * Target:   All users who have completed at least one quiz attempt
 *
 * Usage:
 *   node --env-file=.env.local scripts/send-wc-quiz-day2.mjs          # dry run
 *   node --env-file=.env.local scripts/send-wc-quiz-day2.mjs --send   # fire
 */
...
const LOCK_FILE = "/tmp/yourscore-send-wc-quiz-day2.lock";

async function main() {
  console.log(`\n🧠 YourScore — WC Quiz Series: Day 2`);
  console.log(`   Mode:   ${DRY_RUN ? "DRY RUN (no emails sent)" : "⚡ LIVE — emails WILL be sent"}`);
  console.log(`   Target: All quiz players\n`);
  ...
  // 2. Fetch all auth users and match
  ...
  await syncAndBroadcast(CAMPAIGNS_KEY, {
    audienceName: `WC Quiz Series Day 2 — quiz players (${new Date().toISOString().slice(0, 10)})`,
    emails: targets.map((u) => ({ email: u.email })),
    name: "WC Quiz Series — Day 2 (quiz players)",
    from: FROM,
    replyTo: REPLY_TO,
    subject: SUBJECT,
    html,
