 ...
 * send-wc-launch.mjs
 *
 * Blast: World Cup launch email (template 14) to ALL users.
 * Uses parallel batch sending — all batches fire simultaneously,
 * so 3,500 emails submit to Resend in ~5-10 seconds.
 *
 * Usage:
 *   node --env-file=.env.local scripts/send-wc-launch.mjs          # dry run
 *   node --env-file=.env.local scripts/send-wc-launch.mjs --send   # fire
 */
...
// --fast: no delay between batches (use only for small lists or with warmed domain)
// default: 45s between batches of 100 → ~30 min for 4,000 users
...
async function renderTemplate() {
  const filePath = path.join(__dirname, "..", "emails", "lifecycle", "14-launch-wc-laliga.html");
...
const LOCK_FILE = "/tmp/yourscore-send-wc-launch.lock";

async function main() {
  console.log(`\n🌍 YourScore — World Cup Launch Blast`);
  console.log(`   Mode:   ${DRY_RUN ? "DRY RUN (no emails sent)" : "⚡ LIVE — emails WILL be sent"}`);
  console.log(`   Target: ALL users\n`);

  // Guard: prevent double-send if another instance is already running
  if (!DRY_RUN) {
    try {
  ...
  const SUBJECT = "The World Cup starts today. Can you go 8-0-0?";
  ...
  await syncAndBroadcast(CAMPAIGNS_KEY, {
    ...
    name: "World Cup Launch (14-wc-launch)",
    from: FROM,
    replyTo: REPLY_TO,
    subject: SUBJECT,
    html,
