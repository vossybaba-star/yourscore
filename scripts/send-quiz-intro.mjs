 ...
 * send-quiz-intro.mjs
 *
 * Campaign: Quiz introduction email (template 16)
 * Target:   Users who have NEVER completed a quiz attempt (not in quiz_attempts)
 *
 * Usage:
 *   node --env-file=.env.local scripts/send-quiz-intro.mjs          # dry run
 *   node --env-file=.env.local scripts/send-quiz-intro.mjs --send   # fire
 */
...
async function renderTemplate() {
  const filePath = path.join(__dirname, "..", "emails", "lifecycle", "16-quiz-intro.html");
...
// ── Lockfile guard ────────────────────────────────────────────────────────────
const LOCK_FILE = "/tmp/yourscore-send-quiz-intro.lock";

async function main() {
  console.log(`\n🧠 YourScore — Quiz Introduction Campaign`);
  console.log(`   Mode:   ${DRY_RUN ? "DRY RUN (no emails sent)" : "⚡ LIVE — emails WILL be sent"}`);
  console.log(`   Target: Users who have NEVER played a quiz\n`);
  ...
  // 1. Get all user_ids who HAVE played a quiz
  ...
  console.log(`   ${quizPlayerIds.size} users have already played a quiz — excluding them\n`);

  // 2. Fetch all auth users and keep only those who have NOT played
  ...
  console.log(`   ${allAuthUsers.length} total auth users`);
  ...
  console.log(`   Sending to ${targets.length} non-quiz users\n`);
  ...
  const SUBJECT = "There's a quiz on YourScore. You haven't tried it yet.";
  const html = await renderTemplate();

  await syncAndBroadcast(CAMPAIGNS_KEY, {
    audienceName: `Quiz Intro — never played (${new Date().toISOString().slice(0, 10)})`,
    emails: targets.map((u) => ({ email: u.email })),
    name: "Quiz Intro — never played",
    from: FROM,
    replyTo: REPLY_TO,
    subject: SUBJECT,
    html,
